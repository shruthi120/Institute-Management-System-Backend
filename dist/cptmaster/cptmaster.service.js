...
let CptmasterService = class CptmasterService {
    constructor(cptmasterModel, modifierModel) {
    ...
    async update_cptmaster(id, body) {
        const cptmaster = await this.cptmasterModel.findByIdAndUpdate(id, body, { new: true });
        return cptmaster;
    }
    ...
    async get_by_cptmaster_code(query) {
        if (query) {
            const cptmaster = await this.modifierModel.find({
                cpt_code: { $regex: query, $options: 'i' },
            });
            return cptmaster;
        }
        else {
    ...
    async get_by_modifiercode(query) {
        if (query) {
            const modifier = await this.modifierModel.find({
...
CptmasterService = __decorate([
    ...
    __param(1, mongoose_1.InjectModel('Modifier')),
